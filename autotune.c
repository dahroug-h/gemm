/*
 * autotune.c  —  Parallel in-process auto-tuner for large sizes
 *
 * Tunes for N=256..4000 — the range your large kernel dominates.
 * NPAR is detected at runtime from available CPUs (works on any
 * machine GitHub gives: 2-core Xeon or 64-core EPYC).
 *
 * For small-size tuning (N=100-800) use autotune_small.c instead.
 *
 * Build:
...
/* ── thread-local MC/NC/KC — each bench thread has its own ── */
__thread int MC = 80;
...
static void *bench_thread(void *arg)
{
    ...
    /* pin to specific core for clean measurement */
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
...
/* harmonic mean score across all TUNE_SIZES */
...
int main(void)
{
    /* detect available CPUs — works on any machine */
    int ncpus = (int)sysconf(_SC_NPROCESSORS_ONLN);
    NPAR = ncpus;  /* use ALL available cores for parallel tuning */
    if (NPAR > 64) NPAR = 64;  /* safety cap */
    if (NPAR < 1)  NPAR = 1;

    printf("==============================================\n");
    printf("  int8 GEMM Auto-Tuner (large sizes, parallel)\n");
    printf("  MR=%d  NR=%d\n", MR, NR);
    printf("  CPUs detected: %d  Parallel combos: %d\n", ncpus, NPAR);
    printf("  Tune sizes:");
    ...
    /* ════════════════════════════════════════
     * PHASE 1 — coarse, single size N=1024
     * ════════════════════════════════════════ */
    ...
    printf("══ PHASE 2: %d fine combos (parallel=%d) ══\n", total_f, NPAR);
    printf("   Sizes:");
    for (int s = 0; s < N_TUNE_SIZES; s++) printf(" %d", TUNE_SIZES[s]);
    printf("\n");
    ...
    double best    = -1.0;
    {
        int tc[1][3] = {{c_best_mc, c_best_nc, c_best_kc}};
        double ts[1];
        score_fine_batch(tc, ts, 1);
        best = ts[0];
    }
    ...
    printf("\n==============================================\n");
    printf("  FINAL BEST  (harmonic mean N=%d..%d)\n",
